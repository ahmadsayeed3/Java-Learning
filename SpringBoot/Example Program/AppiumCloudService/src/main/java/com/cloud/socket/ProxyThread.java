...
import com.cloud.enums.HttpProtocol;
...
public class ProxyThread extends Thread {
	...
	private void readHeaderAndBody() throws IOException, ParseException {
		// Read Headers ===
		...
		// Process Headers ===
		this.header = readHeader(eclipseInReader);
		ParsedRequestHeader header = ParsedRequestHeader.parse(this.header);

		// Read Body ===
		this.body = readBody(eclipseInReader, header);
	}
...
//				+ "Host: 127.0.1.1:9400\n" + "Connection: Keep-Alive\n" + "Accept-Encoding: gzip";
	...
	private String readBody(BufferedReader eclipseInReader, ParsedRequestHeader header) throws IOException {
		...
		while (header.Protocol == HttpProtocol.POST && true) {
			...
			if (charactersRead < ProxyThread.BUFFER_SIZE) {
				break;
				// String bodyString = new String(baisBody.toByteArray(), StandardCharsets.UTF_8);
				// this.validateAndReplaceBody(bodyString, null, header);
				// throw new ConnectError("Halted");
			}
		}
